...
import { ecrrepositoryBackendPlugin } from './plugin';
...
describe('plugin', () => {
  it('should create and read TODO items', async () => {
    const { server } = await startTestBackend({
      features: [ecrrepositoryBackendPlugin],
    });

    await request(server).get('/api/ecrrepository-backend/todos').expect(200, {
      items: [],
    });

    const createRes = await request(server)
      .post('/api/ecrrepository-backend/todos')
      .send({ title: 'My Todo' });
    ...
    await request(server)
      .get('/api/ecrrepository-backend/todos')
      .expect(200, {
        items: [createdTodoItem],
      });

    await request(server)
      .get(`/api/ecrrepository-backend/todos/${createdTodoItem.id}`)
